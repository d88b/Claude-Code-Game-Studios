...
func _create_mvp_build_items() -> void:
	...
	for i in range(1000, 1004):
		...
		wall.item_id = i
		wall.display_name = LocalizationManager.translate("build.wall.display_name") + " " + str(i)
		wall.category = "wall"
	...
	for i in range(2000, 2003):
		...
		turret.item_id = i
		turret.display_name = LocalizationManager.translate("build.turret.display_name") + " " + str(i)
	...
	trap.item_id = 3000
	trap.display_name = LocalizationManager.translate("build.trap.display_name")
	trap.category = "trap"
	...
	for i in range(4000, 4003):
		...
		facility.item_id = i
		facility.display_name = LocalizationManager.translate("build.facility.display_name") + " " + str(i)
	...
	floor.item_id = 5000
	floor.display_name = LocalizationManager.translate("build.floor.display_name")
